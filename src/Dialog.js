...
class Dialog extends Component {
  ...
  handlePicked(activity) {
    ...
    this.setState({activityChosen: activity}, () => {
      ...
      const { triggerRerender } = this.props;
      triggerRerender();
    });
  ...
  organizeData() {
    let { title, activities, removeActivity } = this.props;
    let builtDiv = [];
    let occurenceMap = {};
    if (title == 'Requested Activities') {
  ...
  formDialogContent() {
    ...
    } else if (volunteer || activityChosen.length > 0) {
      console.log('rerender activity chosen', activityChosen);
  ...
  render() {
    ...
    const activitiesListing = title.indexOf("Activities") > -1 && (activities.length || Object.keys(activities).length);
    return (
